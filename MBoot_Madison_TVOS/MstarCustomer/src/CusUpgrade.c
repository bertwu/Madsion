...
static int check_bin_flag(void)
{
    ...
    UBOOT_TRACE("IN\n");
#if (ENABLE_MSTAR_BD_MST036B_EDISON_CUS19 == 1)
    if(check_file_partition(force_upgrade_path,NULL,NULL)==0)
#else
...
int do_custar(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
#if (ENABLE_MSTAR_BD_MST036B_EDISON_CUS19 == 1)
    force_upgrade_path = getenv("ForceUpgradePath");
    printf("path = %s\n",force_upgrade_path);
    if(force_upgrade_path == NULL)
    {
        printf("get force upgrade path FAIL!\n");
        return -1;
    }
    ...
    else if(state&UPGRADE_STATE_AP_BIN)//upgrade Supernova
    {
        memset(cmd, 0, 128);
#if (ENABLE_MSTAR_BD_MST036B_EDISON_CUS19 == 1)
        snprintf(cmd, sizeof(cmd)-1, "ustar %s",force_upgrade_path);
#else

...
static U32 NCTEST_RWMultiplePages_OneTimeADMA(U32 u32_StartRowIdx)
{
	...
	//NC_Config();
	#if 1
	u32_Err = NC_ADMAReadPages(u32_StartRowIdx, (U8*)pu8_DesTable, (U8*)au8_RSpare, u32_PageCnt);
	if (u32_Err != UNFD_ST_SUCCESS)
	{
		nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "ERROR: RWMultiplePages_OneTime 3, ErrCode: 0x%08lx\n", u32_Err);
		return u32_Err;
...
//	au8_nand_clk_max = EMMC_PLL_1XCLK_100M_IDX;
	...
	#if (0 || FCIE_IP_TEST_SUITE_ENABLE) && (defined(FCIE_ZDEC_TEST) && FCIE_ZDEC_TEST)
	nand_debug(0, 1, "ZDEC Test...\n");
	u32_Err = NCTEST_ZDEC(u32_TestRow);
	if (u32_Err != UNFD_ST_SUCCESS)
	{
		nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "ERROR: ZDEC Test fail, ErrCode: 0x%08lx\n", u32_Err);
		goto LABEL_END_OF_TESTS;
	}
	nand_debug(0, 1, "ZDEC Test 1 ok!...\n");
	
	nand_debug(0, 1, "ZDEC Test 2...\n");
	u32_Err = NCTEST_ZDEC2(u32_TestRow);
	if (u32_Err != UNFD_ST_SUCCESS)
	{
		nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "ERROR: ZDEC Test 2 fail, ErrCode: 0x%08lx\n", u32_Err);
		goto LABEL_END_OF_TESTS;
	}	
	nand_debug(0, 1, "ZDEC Test 2 ok!...\n");

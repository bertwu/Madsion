    ...
    else
    {
       if(strncmp(argv[1], "init", 5) == 0)
       {
            ...
            if(ret == TRUE)
                ...
                ret = -1;
            //MsDrv_VE_init(u8ResolutionEnv);

        #if(CONFIG_MSTAR_NAPOLI_BD_MST051D_20ANV == 1)
			run_command("delay 9000" , 0);
        #endif
...
#if 0
       else if (strncmp(argv[1], "dini", 5) == 0)
       {
            ret = hdmiExit();              
       }
#endif

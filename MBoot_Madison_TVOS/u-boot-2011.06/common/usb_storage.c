...
#define USB_STOR_TRANSPORT_NODEV  -3

extern int FirstBulkIn,FirstBulkOut;
...
static int us_one_transfer(struct us_data *us, int pipe, char *buf, int length)
{
	...
	int this_xfer=0;
	int result=0;
	int partial=0; //Coverity CID :6460
	int maxtry=0;
	int stat=0;
...
int usb_stor_BBB_clear_endpt_stall(struct us_data *us, __u8 endpt)
{
	...
	if (UsbPortSelect==4)
		ms_usb_reset_ept(us->pusb_dev, endpt);
#endif
	return result;
}
...
int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
	...
	if (result < 0) {
		USB_STOR_PRINTF("failed to send CBW status %ld\n",
			us->pusb_dev->status);
		if (USB_STOR_TRANSPORT_NODEV == result)
			return result;
		usb_stor_BBB_reset(us);
		return USB_STOR_TRANSPORT_FAILED;
	}
        //wait_ms(5); //20120420, remove it for Cruzer 1GB disk
	...
	if ((result < 0) && (us->pusb_dev->status & USB_ST_STALLED)) {
		...
		result = usb_stor_BBB_clear_endpt_stall(us,
					dir_in ? us->ep_in|0x80 : us->ep_out);
		if (result >= 0)
		{
			if (dir_in)
				FirstBulkIn = 1;
...
again:
	...
	if ((result < 0) && (retry < 1) &&
		...
		/* clear the STALL on the endpoint */
		result = usb_stor_BBB_clear_endpt_stall(us, us->ep_in|0x80);
		if ((result >= 0) && (retry < 1))
		{
...
static int usb_test_unit_ready(ccb *srb, struct us_data *ss)
{
	...
	do {
		...
		if(ret==USB_STOR_TRANSPORT_NODEV) {
			return -1;
		}
...
retry_it:
#if 0 //Colin, 100816, for performance test
		if (smallblks == USB_MAX_READ_BLK)
			usb_show_progress();
#endif
		srb->datalen = usb_dev_desc[device].blksz * smallblks;
		srb->pdata = (unsigned char *)buf_addr;
		ret=usb_read_10(srb,(struct us_data *)dev->privptr, start, smallblks);

		if(ret) {
			USB_STOR_PRINTF("Read ERROR\n");
			if (USB_STOR_TRANSPORT_NODEV == ret)
				return 0;
...
retry_it:
		...
		srb->datalen = usb_dev_desc[device].blksz * smallblks;
		srb->pdata = (unsigned char *)buf_addr;
		ret=usb_write_10(srb,(struct us_data *)dev->privptr, start, smallblks);
		if(ret) {
		      ...
		      block_dev_desc_t *dev_desc)
{
	...
	// Skip all not Direct-Access devices
	if ( (perq & 0x1f) != 0 ) {
	...
	/* to avoid mis-sync definition between filesystem calc unit and blk dev R/W unit,
	 * only support 512 bytes block size storage dev,
	 * most filesystem (ext. FAT) only support 512 bytes per sector.
	 */
	if (*blksz != 0x200) {
		printf("[USB] Error!! unsupported block size 0x%lx\n", *blksz);
		return -1;
	}

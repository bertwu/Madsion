...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	case IH_COMP_NONE:
            if (load == blob_start || load == image_start) {
                ...
                if(E_BDMA_OK != MDrv_BDMA_MemCopy(TO_PHY_ADDR(image_start), TO_PHY_ADDR(load), image_len))
                {
...
//Mstar Start
    ...
    Points_Write(PiuTime);
        ...
        #if ((ENABLE_MODULE_TEE==0)&&(ENABLE_MODULE_MSTAR_CLEANBOOT==0))
        unsigned addr = simple_strtoul(argv[1], NULL, 16);
        if(isBootToRecovery()){
			RawData_Authentication("recovery",addr);
        }else{
            RawData_Authentication("boot",addr);
        }
        #endif //#if(!((ENABLE_MODULE_TEE==1)&&(ENABLE_MODULE_MSTAR_CLEANBOOT==1)))
        #else
...
//Mstar Start
#if  (ENABLE_PANEL_INIT==1 && ENABLE_MODULE_TEE == 0 )
	{

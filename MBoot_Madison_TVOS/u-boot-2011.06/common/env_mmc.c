...
{
    uint blk_start, blk_cnt, n, total_size,i;
	...
	blk_start  = mpart->start_block + mpart->block_count - (blk_cnt <<1) + (offset >> 9);
	total_size = mpart->block_count <<9;
    if ((blk_start < 0) || ((size << 1) > total_size))
    {
        printf("%s: Search MPOOL Partition Info failed, at %d\n", __func__, __LINE__);
        return -1;
    }

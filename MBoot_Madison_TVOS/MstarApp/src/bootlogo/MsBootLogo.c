...
#else
    if(get_addr_from_mmap("E_MMAP_ID_GOP_FB", &u32Addr)!=0)
    {
        UBOOT_ERROR("get E_MMAP_ID_GOP_FB mmap fail\n");
        ...
        if (strstr(cmdline, "delaylogo=true") != NULL && (!isBootToRecovery()))
        {
            ...
            if (get_mmap(bootlogo_buffer, &mmapinfo) == 0)
            {
                if (mmapinfo.u32Size > BOOTLOGO_BUFFER_SIZE)
                {
                    ...
                    g_logoMiuIdx = (mmapinfo.b_is_miu0) ? 0 : 1;
                    bDelayLogo = TRUE;
                }
                else
                {
...
int do_bootlogo (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
#else
    if(get_addr_from_mmap("E_MMAP_ID_GOP_FB", &JPD_InputADDR)!=0)
    {
        UBOOT_DEBUG("get E_MMAP_ID_GOP_FB mmap fail\n");
    ...
    if(get_addr_from_mmap("E_MMAP_ID_JPD_WRITE", &JPD_OUTADDR)!=0)
    {

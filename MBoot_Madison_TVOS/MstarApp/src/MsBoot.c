...
#define _VA2NVA(x) ((((U32)x)^CONFIG_SYS_MIU0_CACHE)|CONFIG_SYS_MIU0_NON_CACHE)
...
#endif
#if defined(CONFIG_MSTAR_EDISON )|| defined(CONFIG_MSTAR_EIFFEL) || defined(CONFIG_MSTAR_NUGGET) || defined(CONFIG_MSTAR_EINSTEIN)|| defined(CONFIG_MSTAR_NAPOLI)
...
//	_Write2Byte(0x101EA6, 0x0D04);
	...
	// Set to 240 MHz
#if defined(CONFIG_MSTAR_EDISON) || defined(CONFIG_MSTAR_EIFFEL) || defined(CONFIG_MSTAR_EINSTEIN)|| defined(CONFIG_MSTAR_NAPOLI)
...
U32 MoveNuttx2Dest(U32 NuttxVirtMallocAddr, U32 NuttxSize)
{
    ...
    UBOOT_DUMP(_VA2NVA(NuttxVirtMallocAddr), 0x100);
    
    pstTeeHd=(ST_TEE_HEADER*)NuttxVirtMallocAddr;
    UBOOT_DEBUG("NuttxVirtMallocAddr=%x\n",(U32)NuttxVirtMallocAddr);
    ...
    if(memcmp(pstTeeHd->ucMagicID,TeeMagicID,strlen(TeeMagicID))==0)
    {
        ...
        UBOOT_DEBUG("pstTeeHd->uitee_start_paddr=%x\n",pstTeeHd->_sthd.uitee_start_paddr);
        UBOOT_DEBUG("pstTeeHd->uitee_start_vaddr=%x\n",pstTeeHd->_sthd.uitee_start_vaddr);
        UBOOT_DEBUG("pstTeeHd->uitee_image_size=%d\n",pstTeeHd->_sthd.uitee_image_size);
        UBOOT_DEBUG("pstTeeHd->ulltime=%llx\n",pstTeeHd->_sthd.ulltime);
        UBOOT_DEBUG("pstTeeHd->ullchange_list=%llx\n",pstTeeHd->_sthd.ullchange_list);
        UBOOT_DEBUG("pstTeeHd->uiversion=%d\n",pstTeeHd->_sthd.uiversion);
        ...
        u32NuttxPhyAddr=(NuttxVirtMallocAddr^CONFIG_SYS_MIU0_CACHE);
        ...
        UBOOT_DEBUG("u32R2PhyAddr|NONCACHE_ADDR=%x\n",u32NuttxPhyAddr|CONFIG_SYS_MIU0_NON_CACHE);
    ...
    else
    {
        ...
        NuttxPhyMallocAddr=NuttxVirtMallocAddr^CONFIG_SYS_MIU0_CACHE;
...
int do_readNuttx (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    memset(buffer,0,CMD_BUF);
	#if(ENABLE_MMC==1)
    snprintf(buffer,CMD_BUF, "mmc read.p %x tee %x",pu8NuttxHeaderAddr,NUTTX_HEADER_LEN);
	#elif (ENABLE_NAND_FLASH==1)
    snprintf(buffer,CMD_BUF, "nand read.e %x tee %x",pu8NuttxHeaderAddr,NUTTX_HEADER_LEN);
	#else
		#error
	#endif
    UBOOT_DEBUG("cmd: %s \n",buffer);
    ...
    if(NuttxAlignSize!=0)
    {
        ...
        memset(buffer,0,CMD_BUF);
		#if(ENABLE_MMC==1)
        snprintf(buffer,CMD_BUF, "mmc read.p %x tee %x",pNuttxAlignMallocAddr,NuttxAlignSize);
		#elif(ENABLE_NAND_FLASH==1)
        snprintf(buffer,CMD_BUF, "nand read.e %x tee %x",pNuttxAlignMallocAddr,NuttxAlignSize);
		#else
			#error
		#endif
        UBOOT_DEBUG("cmd: %s \n",buffer);
    ...
    else
    {
        ...
        memset(buffer,0,CMD_BUF);
		#if(ENABLE_MMC==1)
        snprintf(buffer,CMD_BUF, "mmc read.p %x tee %x",pNuttxAlignMallocAddr,TEE_PART_SIZE);
		#elif(ENABLE_NAND_FLASH==1)
		snprintf(buffer,CMD_BUF, "nand read.e %x tee %x",pNuttxAlignMallocAddr,TEE_PART_SIZE);
		#else
			#error
		#endif
        UBOOT_DEBUG("cmd: %s \n",buffer);
...
int do_bootNuttx (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if(argc==2)
    {
        ...
        u32NuttxVirtAddr = (u32NuttxPhyAddr|CONFIG_SYS_MIU0_CACHE);
    }
    else
    {
...
#if defined (CONFIG_SECURITY_BOOT)
        if(-1==RawData_Authentication("tee",u32NuttxVirtAddr)){
		UBOOT_ERROR("TEE Authentication fail\n");
		return -1;
	}
#endif

    UBOOT_DUMP(_VA2NVA(u32NuttxVirtAddr), 0x100);
    ...
    u32CustomerKeyBankAddr=(CONFIG_SYS_MIU0_NON_CACHE|u32HwAesPABufAddr);
    u32NConfigAddr=u32CustomerKeyBankAddr+sizeof(CUSTOMER_KEY_BANK);
    gu32TeeBootConfigAddr=(CONFIG_SYS_MIU0_NON_CACHE|u32HwAesPABufAddr)-NUTTX_BOOT_CONFIG_SIZE;
...
int do_load_nconfig(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    u32HwAesVABufAddr=(CONFIG_SYS_MIU0_NON_CACHE|u32HwAesPABufAddr);
...
#if defined(CONFIG_MSTAR_NAPOLI_BD_MST189A_D01A_S)
